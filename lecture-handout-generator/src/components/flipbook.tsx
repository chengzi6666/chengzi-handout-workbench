...
function PageContent({ page, pageNumber }: { page?: Record<string, unknown>; pageNumber?: string }) {
  ...
  return <>
    ...
    {Array.isArray(page.practice) ? <div>{(page.practice as Array<{ prompt: string; answer: string; imageUrl?: string }>).map((item, index) => <section key={index}><b>{index + 1}. {item.prompt}</b>{item.imageUrl ? <img className="book-question-image" src={item.imageUrl} alt={`第${index + 1}题题图`} /> : null}{showAnswers ? <p><b>参考作答：</b>{item.answer}</p> : <p>我的作答：＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿</p>}</section>)}</div> : null}
...
export function Flipbook({ title, description, pages, coverSrc, shareCoverSrc, projectId }: { title: string; description: string; pages: Array<Record<string, unknown>>; coverSrc?: string; shareCoverSrc?: string; projectId?: string }) {
