...
export function parentPageSpec(grade: string, lessons: LessonContent[], teacherName = "主讲", teacherIntroduction = "负责阅读方法、表达写作和课堂互动引导。"): SharedPage[] {
  return [
    { role: "PARENT_MANUAL", blocks: [{ kind: "title", text: "家长使用手册" }, { kind: "text", text: "—— 真读书 · 有深度 · 用得上 ——" }, { kind: "heading", text: `${teacherName}老师｜主讲老师` }, { kind: "text", text: teacherIntroduction }, { kind: "heading", text: "🤝 双师陪伴｜主讲老师＋班主任老师" }, { kind: "callout", text: "", items: [`${teacherName}老师负责课程讲解、阅读方法和表达写作训练；班主任老师负责直播跟课、日常答疑、阶段反馈、薄弱点跟踪和学习规划，两位老师共同陪伴一个孩子。`] }] },
    { role: "PARENT_MANUAL", blocks: [{ kind: "title", text: "五讲课程带来的能力提升" }, { kind: "text", text: "五讲合起来，孩子练习的是：读懂故事 → 找到证据 → 学会方法 → 说清楚 → 写完整。" }, { kind: "heading", text: "五讲学习安排" }, ...lessons.map((lesson) => ({ kind: "callout" as const, text: `第${lesson.lessonNumber}讲｜${lesson.title}｜${lesson.technique}`, items: lesson.learningGoals.map((goal, index) => `${index + 1}. ${goal}`) }))] },
