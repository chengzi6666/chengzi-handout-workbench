...
export async function extractPdfPages(
  ...
  try {
    const { stdout } = await run(executable("pdfinfo"), [inputPath], { maxBuffer: 4 * 1024 * 1024 });
    ...
    for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
      ...
      await run(executable("pdftotext"), ["-f", String(pageNumber), "-l", String(pageNumber), "-layout", "-enc", "UTF-8", inputPath, textPath], { maxBuffer: 8 * 1024 * 1024 });
      await run(executable("pdftoppm"), ["-f", String(pageNumber), "-l", String(pageNumber), "-singlefile", "-r", "130", "-png", inputPath, imagePrefix], { maxBuffer: 8 * 1024 * 1024 });
