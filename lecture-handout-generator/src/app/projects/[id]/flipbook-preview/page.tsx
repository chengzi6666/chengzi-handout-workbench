...
export default async function FlipbookPreviewPage({
  ...
  const parentPages = [
    ...
    { collection: "parent", kind: "parent", title: "五讲课程带来的能力提升", richHtml: richPage(project.layoutConfig, "parent", 0, 1), backgroundSrc: parentBackground, body: ["五讲合起来，孩子练习的是：读懂故事 → 找到证据 → 学会方法 → 说清楚 → 写完整。", "五讲学习安排", ...project.lessons.map((savedLesson) => { const lesson = lessonContentSchema.parse(savedLesson.structuredContent); return `第${savedLesson.lessonNumber}讲《${lesson.title}》｜${lesson.technique}｜${lesson.learningGoals.map((goal) => goal.replace(/^我[们]?/u, "")).join("；")}`; })] },
  ...
  const answerPages = project.lessons.flatMap((savedLesson) => {
    const lesson = lessonContentSchema.parse(savedLesson.structuredContent);
    return [
      { collection: "answers", kind: "answer", title: `第${savedLesson.lessonNumber}讲参考答案`, topics: lesson.conversationTopics, richHtml: richPage(project.layoutConfig, "answers", savedLesson.lessonNumber, 0), backgroundSrc: pageBackground(project, "SIMPLE") },
