...
export default async function FlipbookPreviewPage({
  ...
  const parentPages = [
    ...
    { collection: "parent", kind: "parent", title: "五讲课程带来的能力提升", richHtml: richPage(project.layoutConfig, "parent", 0, 1), backgroundSrc: parentBackground, body: ["五讲合起来，孩子练习的是：读懂故事 → 找到证据 → 学会方法 → 说清楚 → 写完整。", "五讲合起来，孩子练习的是：", ...project.lessons.map((savedLesson) => { const lesson = lessonContentSchema.parse(savedLesson.structuredContent); return `第${savedLesson.lessonNumber}讲：${lesson.technique}`; })] },
    { collection: "parent", kind: "parent", title: "五讲学习安排", richHtml: richPage(project.layoutConfig, "parent", 0, 2), backgroundSrc: parentBackground, body: project.lessons.map((savedLesson) => { const lesson = lessonContentSchema.parse(savedLesson.structuredContent); return `第${savedLesson.lessonNumber}讲｜${lesson.title}｜${lesson.technique}｜${lesson.learningGoals.map((goal) => goal.replace(/^我[们]?/u, "")).join("；")}`; }) },
    { collection: "parent", kind: "parent", title: `🎯 ${project.grade}阶段，最需要关注什么？`, richHtml: richPage(project.layoutConfig, "parent", 0, 3), backgroundSrc: parentBackground, body: ["基础：从“会认字”走向“会用字词”——在故事语境中认识并积累字词，并能用到自己的口头和书面表达中。", "阅读：从“听故事”走向“读懂故事”——说清人物、事情、结果与道理，并从原文中找到具体词句作证据。", "表达：从“说一句话”走向“完整表达”——借助课堂方法，把人物、事情、动作、语言、心情和结果说完整、写清楚。", "💡 家长怎么配合？正课前后按讲义完成复述、笔记或书面练习，并由班主任给予跟踪反馈。"] }
  ];
