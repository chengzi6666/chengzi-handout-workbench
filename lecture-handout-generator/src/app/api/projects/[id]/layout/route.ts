...
const configSchema = z.object({ teacherImage: z.object({ assetId: z.string().optional(), x: z.number().min(0).max(100), y: z.number().min(0).max(100), width: z.number().min(5).max(80), height: z.number().min(5).max(80) }), fontFamily: z.enum(["Microsoft YaHei", "SimSun", "KaiTi", "FangSong"]).default("Microsoft YaHei"), fontSize: z.number().min(10).max(18).default(11), headerText: z.string().max(80).optional(), footerText: z.string().max(80).optional(), richPreviewHtml: z.record(z.string(), z.string().max(100000)).optional() });
