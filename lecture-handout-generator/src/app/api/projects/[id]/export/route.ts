...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const layout = project.layoutConfig as { teacherImage?: { assetId?: string; x: number; y: number; width: number; height: number }; fontSize?: number; fontFamily?: "Microsoft YaHei" | "SimSun" | "KaiTi" | "FangSong"; headerText?: string; headerSize?: number; footerText?: string; footerSize?: number; noteOwnPage?: boolean; richPreviewHtml?: Record<string, string> } | null;
  ...
  const buffer = await generateHandoutDocx({ projectName: project.name, grade: project.grade, teachingYear: project.teachingYear, teacherFormalName: project.teacher?.formalName, teacherNickname: project.teacher?.nickname, teacherIntroduction: project.teacher?.introduction ?? undefined, headerText: layout?.headerText, headerSize: layout?.headerSize, footerText: layout?.footerText, footerSize: layout?.footerSize, noteOwnPage: layout?.noteOwnPage, richPreviewHtml: layout?.richPreviewHtml, lessons: lessons.map((item) => item.content), pinyinReviews, backgrounds, teacherImage, teacherPortrait, teacherPosition: layout?.teacherImage, fontSize: layout?.fontSize, fontFamily: layout?.fontFamily, practiceImages, includeParentManual: kind === "combined_parent_student", mode });
