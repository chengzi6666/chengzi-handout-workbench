...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const layout = project.layoutConfig as { teacherImage?: { assetId?: string; x: number; y: number; width: number; height: number }; fontSize?: number; fontFamily?: "Microsoft YaHei" | "SimSun" | "KaiTi" | "FangSong"; headerText?: string; footerText?: string; noteOwnPage?: boolean } | null;
  ...
  const buffer = await generateHandoutDocx({ projectName: project.name, grade: project.grade, teachingYear: project.teachingYear, teacherFormalName: project.teacher?.formalName, teacherNickname: project.teacher?.nickname, headerText: layout?.headerText, footerText: layout?.footerText, noteOwnPage: layout?.noteOwnPage, lessons: lessons.map((item) => item.content), pinyinReviews, backgrounds, teacherImage, teacherPortrait, teacherPosition: layout?.teacherImage, fontSize: layout?.fontSize, fontFamily: layout?.fontFamily, practiceImages, includeFrontMatter: kind === "combined_student", mode });
