...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parent = [
    ...
    { collection: "parent", kind: "parent", title: "五讲课程带来的能力提升", richHtml: richPage(project.layoutConfig, "parent", 0, 1), backgroundSrc: background("PARENT_MANUAL"), body: ["五讲合起来，孩子练习的是：读懂故事 → 找到证据 → 学会方法 → 说清楚 → 写完整。", "五讲学习安排", ...lessons.map((lesson) => `第${lesson.lessonNumber}讲《${lesson.title}》｜${lesson.technique}｜${lesson.learningGoals.map((goal) => goal.replace(/^我[们]?/u, "")).join("；")}`)] },
  ...
  const answers = lessons.flatMap((lesson) => [
    { collection: "answers", kind: "answer", title: `第${lesson.lessonNumber}讲参考答案`, topics: lesson.conversationTopics, richHtml: richPage(project.layoutConfig, "answers", lesson.lessonNumber, 0), backgroundSrc: background("SIMPLE") },
