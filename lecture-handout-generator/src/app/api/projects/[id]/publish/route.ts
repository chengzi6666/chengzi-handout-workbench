...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parent = [
    { collection: "parent", kind: "parent", title: "家长使用手册", subtitle: "—— 真读书 · 有深度 · 用得上 ——", teacherPortraitSrc, richHtml: richPage(project.layoutConfig, "parent", 0, 0), backgroundSrc: background("PARENT_MANUAL"), body: [`${teacherName}老师｜主讲老师`, project.teacher?.introduction ?? "负责阅读方法、表达写作和课堂互动引导。", "🤝 双师陪伴｜主讲老师＋班主任老师", `${teacherName}老师负责课程讲解、阅读方法和表达写作训练；班主任老师负责直播跟课、日常答疑、阶段反馈、薄弱点跟踪和学习规划，两位老师共同陪伴一个孩子。`] },
    { collection: "parent", kind: "parent", title: "五讲课程带来的能力提升", richHtml: richPage(project.layoutConfig, "parent", 0, 1), backgroundSrc: background("PARENT_MANUAL"), body: ["五讲合起来，孩子练习的是：读懂故事 → 找到证据 → 学会方法 → 说清楚 → 写完整。", "五讲合起来，孩子练习的是：", ...lessons.map((lesson) => `第${lesson.lessonNumber}讲：${lesson.technique}`)] },
    { collection: "parent", kind: "parent", title: "五讲学习安排", richHtml: richPage(project.layoutConfig, "parent", 0, 2), backgroundSrc: background("PARENT_MANUAL"), body: lessons.map((lesson) => `第${lesson.lessonNumber}讲｜${lesson.title}｜${lesson.technique}｜${lesson.learningGoals.map((goal) => goal.replace(/^我[们]?/u, "")).join("；")}`) },
    { collection: "parent", kind: "parent", title: `🎯 ${project.grade}阶段，最需要关注什么？`, richHtml: richPage(project.layoutConfig, "parent", 0, 3), backgroundSrc: background("PARENT_MANUAL"), body: ["基础：从“会认字”走向“会用字词”——在故事语境中认识并积累字词，并能用到自己的口头和书面表达中。", "阅读：从“听故事”走向“读懂故事”——说清人物、事情、结果与道理，并从原文中找到具体词句作证据。", "表达：从“说一句话”走向“完整表达”——借助课堂方法，把人物、事情、动作、语言、心情和结果说完整、写清楚。", "💡 家长怎么配合？正课前后按讲义完成复述、笔记或书面练习，并由班主任给予跟踪反馈。"] },
  ];
  const student = lessons.flatMap((lesson) => [
    { collection: "student", kind: "home", title: `第${lesson.lessonNumber}讲 ${lesson.title}`, subtitle: lesson.subtitle, body: lesson.learningGoals, technique: lesson.technique, richHtml: richPage(project.layoutConfig, "student", lesson.lessonNumber, 0), backgroundSrc: background("LESSON_HOME") },
    { collection: "student", kind: "conversation", title: "课后交流话题", topics: lesson.conversationTopics, richHtml: richPage(project.layoutConfig, "student", lesson.lessonNumber, 1), backgroundSrc: background("CONVERSATION") },
    { collection: "student", kind: "reading", title: "阅读文段", text: lesson.readingExcerpt.text, pinyinUnits: project.grade === "1升2" ? (() => { const row = project.lessons.find((item) => item.lessonNumber === lesson.lessonNumber); try { return row?.pinyinReview ? validatePinyinReview(lesson.readingExcerpt.text, row.pinyinReview as Array<{ char: string; pinyin: string }>) : createPinyinReview(lesson.readingExcerpt.text); } catch { return createPinyinReview(lesson.readingExcerpt.text); } })() : undefined, questions: lesson.closeReadingQuestions, richHtml: richPage(project.layoutConfig, "student", lesson.lessonNumber, 2), backgroundSrc: background("READING") },
    { collection: "student", kind: "practice", title: "课堂方法与真题带练", method: lesson.methodSummary, practice: publicPractice(lesson.practice, slug), richHtml: richPage(project.layoutConfig, "student", lesson.lessonNumber, 3), backgroundSrc: background("PRACTICE") },
    { collection: "student", kind: "teacher", title: "我是小老师", steps: lesson.littleTeacherSteps, framework: lesson.oralFramework, richHtml: richPage(project.layoutConfig, "student", lesson.lessonNumber, 4), backgroundSrc: background("LITTLE_TEACHER") }
  ]);
  const answers = lessons.map((lesson) => ({ collection: "answers", kind: "answer", title: `第${lesson.lessonNumber}讲参考答案`, topics: lesson.conversationTopics, practice: publicPractice(lesson.practice, slug), richHtml: richPage(project.layoutConfig, "answers", lesson.lessonNumber, 0), backgroundSrc: background("SIMPLE") }));
