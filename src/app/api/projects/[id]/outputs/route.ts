...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (process.env.LOCAL_DEMO_MODE === "true") return NextResponse.json({ ok: true });
  const project = await db.project.findFirst({ where: { id, ownerId: session.userId } }); if (!project) return NextResponse.json({ error: "项目不存在" }, { status: 404 });
